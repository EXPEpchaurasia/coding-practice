 ...
 * <p>
 * Youtube Tutorial : https://www.youtube.com/watch?v=byPICGyfcB8
 * GeeksForGeeks : https://www.geeksforgeeks.org/burst-balloon-to-maximize-coins/
 */
            ...
            for (int start = 1; start <= baloons.length - length - 1; ++start) {
                ...
                for (int burstPoint = start; burstPoint <= end; ++burstPoint) {
                    maxBenefit[start][end] = Math.max(maxBenefit[start][end],
                            maxBenefit[start][burstPoint - 1] + maxBenefit[burstPoint + 1][end]
